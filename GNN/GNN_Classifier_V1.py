...
def main(argument:str):
    ...
    # instantiate model
    model = NN_Modules.GraphClassifier(GNN_DIM, nn_dim, N_ITER).to(device=device)
    ...
    metrics.visualize_results(list(y_test), list(predictions), list(X_test['domain']),f'V1_{argument.split(".")[0]}.png')
